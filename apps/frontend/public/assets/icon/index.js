// export { default as iPhone } from "../../public/assets/icon/iPhone.svg";
...
const test = requireContext("../", true, /\.(png|jpe?g|svg)$/);
console.log(
  "🚀 ~ file: index.js ~ line 3 ~ test",
  test
    .keys()
    .map((item) => item.split("/")[1])
    .map((item) => {
      return `export {default as ${item
        .replace(".", "_")
        .replace("-", "")
        .toUpperCase()}} from '../../public/assets/${
        item.includes("svg") ? "icon" : "img"
      }/${item}'`;
    })
);
